...
| `/forum` | `src/app/forum/page.js` | Public forum landing. Sticky category rail (9 categories from the `POST_CATEGORIES` map in `src/lib/postCategories.js`, plus an "All categories" entry) — collapses to a top grid on mobile. Main area lists posts fetched from `GET /api/posts` with pagination (20/page, prev/next controls) and a Recent/Popular sort toggle. Each row shows title, category chip, tags, author, relative date, `score`, and a comment count (placeholder `0` until Class 10). Pinned rows carry a "Pinned" badge. A **New Post** button on the top right routes to `/forum/new` when signed in and opens Clerk's sign-in modal when signed out. |
...
| `/forum/[id]` | `src/app/forum/[id]/page.js` | Public post detail. Shows category chip, title, author + date (+ "edited" timestamp when different), tags, body (whitespace preserved), and any uploaded images in a grid. When the current viewer is the author (compares `getMe()._id` against `post.author._id`), Edit and Delete buttons appear; Delete confirms and routes back to `/forum`. Comments section is a placeholder ("Comments coming soon") — real comments arrive in Class 10. Inline 404 UI when the id is malformed or the post is missing. |
