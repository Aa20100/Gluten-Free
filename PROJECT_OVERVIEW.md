...
| `/forum` | `src/app/forum/page.js` | Public forum landing. Sticky category rail (9 categories from `src/lib/postCategories.js` + an "All categories" entry) collapses to a top grid on mobile. Above the list: a **Search** input (backend `q` param, submit-driven), a **Tag** input (submit or blur), and a **Recent / Popular** sort toggle — every filter/sort/paging change updates the URL query string, and the list refetches from `GET /api/posts` via component-key remount. Post rows: title, category chip, tag chips (each is itself a filter link), author, relative date, `score`, comment-count placeholder, and a "Pinned" badge when applicable. A **New Post** button on the top right routes to `/forum/new` when signed in and opens Clerk's sign-in modal when signed out. Prev/next pagination at 20/page. |
| `/forum/new` | `src/app/forum/new/page.js` | Protected create form (middleware gates `/forum/new(.*)`). Fields: Title, Category dropdown (the 9 categories), Tags (comma-separated → deduped array), Body, and optional images via `CloudinaryUploader`. Submit calls `createPost` → navigates to the new post's detail page. |
| `/forum/[id]` | `src/app/forum/[id]/page.js` | Public post detail. Renders category/Pinned/Locked chips, title, author + date, tags (each links back to `/forum?tag=…`), body, image grid. **Vote widget** (▲ score ▼) that reflects the current user's vote — clicking the active button sends `direction: "clear"`, otherwise `"up"` / `"down"`; signed-out clicks open Clerk's sign-in modal; locked posts disable voting. **Author-only** Edit + Delete. **Moderator-only** panel with Pin/Unpin, Lock/Unlock, and moderator Delete. **Report** button on the post and on every comment opens the shared `ReportButton` modal → `POST /api/reports`. **Comments** section: new-comment box at the top (hidden with a lock notice when the post is locked, signed-out CTA otherwise), then comments fetched from `GET /api/comments/post/:id` and grouped into top-level roots with all descendants rendered flat under the root at one level of indent (spec: deeper replies stay at the same indent). Each comment carries author, date, edited-indicator, body, like button (optimistic toggle → `POST /api/comments/:id/like`), Reply button (inline reply form; reply's `parent` is set to the comment id), and author-only Edit/Delete (soft-delete redacts body + author to "[deleted]" in the response). Inline 404 for missing/malformed ids. |
| `/moderation/reports` | `src/app/moderation/reports/page.js` | Moderator-only. Middleware requires sign-in; the page then fetches `getMe()` and shows a friendly "Moderators only" panel for non-moderators (the API also enforces the check). For moderators, lists every report newest-first: target type, timestamp, reason, reporter name, and a "View target →" deep-link (posts link to `/forum/:id`; comments show the raw id since the report doesn't carry the parent post id). |
...
- `CLERK_PUBLISHABLE_KEY` — same string as the frontend's `NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY`. `clerkMiddleware()` needs both keys.
- `CLERK_SECRET_KEY` — Clerk secret key. Server-side signing material — never expose to the client.
- `CLOUDINARY_CLOUD_NAME` — your Cloudinary cloud id.
- `CLOUDINARY_API_KEY` — Cloudinary API key (Settings → API Keys).
- `CLOUDINARY_API_SECRET` — Cloudinary API secret. Server-side only. The backend uses all three to configure the Cloudinary SDK on boot; `POST /api/posts` streams multipart image uploads through the SDK and stores the resulting `secure_url`s in `Post.imageUrls`.
...
Image uploads on the New Post form now route through the backend, so **no frontend Cloudinary env vars are needed**. Files are POSTed to `/api/posts` as multipart, and the backend uploads them to Cloudinary using its own `CLOUDINARY_*` keys — see the Backend section above.
...
- **Class 10** — Full forum interactions on the frontend. `src/lib/api.js` gained 10 wrappers (votePost / pinPost / lockPost / moderatorDeletePost + comments CRUD + likeComment + reportContent + listReports). New reusable `src/components/ReportButton.js` (button + modal, opens Clerk sign-in when signed out, POSTs to `/api/reports`; used on the post header and every comment). Header converted to a client component that fetches `getMe()` once per sign-in to surface a moderator-only "Reports" link (gated on `isSignedIn && isModerator` so a stale role state after sign-out can't leak the link). Middleware now protects `/moderation(.*)`. Extended `/forum` with URL-driven **Search** (`q`) and **Tag** filters alongside the existing sort toggle; every filter change resets `page`. Rebuilt `/forum/[id]` with a full vote widget (click-active-again → `clear`, disabled on locked posts, sign-in modal for guests), nested comments (top-level roots + all descendants flat at one indent per spec), like/reply/edit/delete per comment, new-comment box at the top (or a lock notice on locked posts), Report on post + comments, and a moderator panel (Pin/Unpin / Lock/Unlock / moderator Delete). New `/moderation/reports` page — middleware-gated for sign-in, then role-checked client-side with a friendly "Moderators only" fallback for regular users; lists every report populated with reporter name and a deep-link to the target when it's a post. Live smoke: `/`, `/forum`, `/forum?q=…&tag=…&sort=…`, and `/forum/:id` all 200; `/moderation/reports` unauth → 307 to Clerk. ✅ Done
