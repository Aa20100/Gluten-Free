...
- `NEXT_PUBLIC_CLOUDINARY_CLOUD_NAME` — your Cloudinary cloud name (short identifier from the dashboard's Product Environment). Used for image uploads on the New Post form.
- `NEXT_PUBLIC_CLOUDINARY_UPLOAD_PRESET` — an **unsigned** upload preset (Cloudinary → Settings → Upload → Upload presets → Add, with Signing Mode: Unsigned). Uploads happen **directly from the browser** to `https://api.cloudinary.com/v1_1/<cloud>/image/upload` — no backend involvement. Leave both blank to hide the uploader gracefully; text posts still work.
